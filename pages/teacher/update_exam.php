...
$sql = $con->query("SELECT exams.id as id, subjects.id as subj_id,subject_name, exam_name, exam_doc, marking_scheme, exams.created_at FROM exams, subjects WHERE exams.id = '" . $_GET['id'] . "' AND exams.subject_id = subjects.id");

$row = mysqli_fetch_assoc($sql);

$_SESSION['exam_doc'] = $row['exam_doc'];
die('<h3 class="text-danger card-title">On progress...</h3>')
                                ...
                                <div class="card-body">
                                    ...
                                    ?> <form class="forms-sample" method="POST" action="" enctype="multipart/form-data">
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control" required id="exampleTextarea1" rows="4" name="description"><?= $row['exam_name'] ?></textarea>
                                        </div>
                                        <label for="subject">Subject</label>
                                        <div class="form-group row d-flex justify-content-center align-items-center">
                                            <select required id="subject" class="js-example-basic-single col-md-11 col-sm-11 col-lg-11" name="subject">
                                                <option value="<?= $row['subj_id'] ?>"><?= $row['subject_name']; ?></option>
                                                <?php
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="file" id="exam" name="exam" class="file-upload-default" required value="../../assets/images/exams/<?= $_SESSION['exam_doc']; ?>" />
                                            <div class="input-group col-xs-12">
                                                <input type="text" class="form-control file-upload-info" disabled value="../../assets/images/exams/<?= $_SESSION['exam_doc']; ?>" />
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="file" id="scheme" name="scheme" value="../../assets/images/schemes/<?= $row['marking_scheme']; ?>" class="file-upload-default" required />
                                            <div class="input-group col-xs-12">
                                                <input type="text" class="form-control file-upload-info" disabled placeholder="<?= $row['marking_scheme']; ?>" />
                                                <span class="input-group-append">
                                                    <button class="file-upload-browse btn btn-primary" type="button">
                                                        Choose doc
                                                    </button>
